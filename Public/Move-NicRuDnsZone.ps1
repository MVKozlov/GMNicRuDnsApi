<#
    .SYNOPSIS
        Move domain zone to another service
    .DESCRIPTION
        Перемещение зоны домена с одной услуги DNS-хостинга на другую
    .PARAMETER Service
...
function Move-NicRuDnsZone {
    ...
    $requestParams = @{
        Uri = "https://api.nic.ru/dns-master/services/$Service/zones/$(Get-Punycode $ZoneName)/move/$NewService"
        Headers = $Headers
