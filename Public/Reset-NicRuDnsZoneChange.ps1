<#
    .SYNOPSIS
        Rollback zone records change
    .DESCRIPTION
        Отмена сделанных изменений в зоне
    .PARAMETER Service
...
#>
function Reset-NicRuDnsZoneChange {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Service,
    ...
    $requestParams = @{
        Uri = "https://api.nic.ru/dns-master/services/$Service/zones/$(Get-Punycode $ZoneName)/rollback"
        Headers = $Headers
