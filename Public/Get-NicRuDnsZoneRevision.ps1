...
#>
function Get-NicRuDnsZoneRevision {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Service,
    ...
    $requestParams = @{
        Uri = "https://api.nic.ru/dns-master/services/$Service/zones/$(Get-Punycode $ZoneName)/revisions"
        Headers = $Headers
    ...
    if ($r -and $r.response) {
        if ($r.response.status -eq 'success') {
            $r.response.data.zone.revision | Select-Object number, date, ip
        }
        else {
